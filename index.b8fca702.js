...
game.getState = function() {
    ...
    for(let i = 0; i < 4; i++){
        const row = [];
        for(let j = 0; j < 4; j++)row.push(fieldRow[i].cells[j].textContent);
...
game.moveLeft = function() {
    ...
    for(let i = 0; i < 4; i++)for(let j = 0; j < 4; j++)copyField[i][j] = Number(field[i][j]) === 0 ? "" : Number(field[i][j]);
    for(let i = 0; i < 4; i++){
        let row = [
            ...field[i]
        ].filter((item)=>Number(item) !== 0).map((item)=>Number(item));
        for(let j = 0; j < row.length; j++)if (Number(row[j]) === Number(row[j + 1])) {
        ...
        for(let ite = row.length; ite < 4; ite++)row.push("");
        field[i] = row;
    }
    for(let i = 0; i < 4; i++){
        for(let j = 0; j < 4; j++)if (copyField[i][j] !== field[i][j]) {
            fieldChanged = true;
            break;
        }
    }
...
game.moveRight = function() {
    ...
    for(let i = 0; i < 4; i++){
        ...
        for(let j = row.length - 1; j > 0; j--)if (Number(row[j]) === Number(row[j - 1])) {
            row[j] = Number(row[j]) * 2;
            addScoreIsMove += Number(row[j]);
            row[j - 1] = "";
            j--;
        ...
        row = row.filter((item)=>item !== "");
        for(let ite = row.length; ite < 4; ite++)row.unshift("");
        field[i] = row;
    }
    for(let i = 0; i < 4; i++){
        for(let j = 0; j < 4; j++)if (copyField[i][j] !== field[i][j]) {
...
game.moveDown = function() {
    ...
    for(let i = 0; i < 4; i++){
        ...
        for(let j = 0; j < 4; j++){
            if (j === 0) columns[0].push(row[j]);
            if (j === 1) columns[1].push(row[j]);
            if (j === 2) columns[2].push(row[j]);
            if (j === 3) columns[3].push(row[j]);
        }
    }
    for(let i = 0; i < 4; i++)columns[i] = columns[i].filter((item)=>Number(item) !== 0).map((item)=>Number(item));
    for(let i = 0; i < 4; i++){
        for(let j = columns[i].length - 1; j > 0; j--)if (Number(columns[i][j]) === Number(columns[i][j - 1])) {
        ...
        for(let ite = columns[i].length; ite < 4; ite++)columns[i].unshift("");
    }
    for(let i = 0; i < 4; i++){
        const row = [
            ...field[i]
        ];
        for(let j = 0; j < 4; j++)row[j] = columns[j][i];
        field[i] = row;
    }
    for(let i = 0; i < 4; i++){
        for(let j = 0; j < 4; j++)if (copyField[i][j] !== field[i][j]) {
...
game.moveUp = function() {
    ...
    for(let i = 0; i < 4; i++)columns[i] = columns[i].filter((item)=>Number(item) !== 0).map((item)=>Number(item));
    for(let i = 0; i < 4; i++){
        for(let j = 0; j < columns[i].length; j++)if (Number(columns[i][j]) === Number(columns[i][j + 1])) {
            columns[i][j] = Number(columns[i][j]) * 2;
            addScoreIsMove += Number(columns[i][j]);
            columns[i][j + 1] = "";
            j++;
        ...
        columns[i] = columns[i].filter((item)=>item !== "");
        for(let ite = columns[i].length; ite < 4; ite++)columns[i].push("");
    }
    for(let i = 0; i < 4; i++){
        const row = [
            ...field[i]
...
game.render = function(field) {
    ...
    for(let i = 0; i < 4; i++)for(let j = 0; j < 4; j++)rowState[i].cells[j].className = "field-cell";
    for(let i = 0; i < 4; i++)for(let j = 0; j < 4; j++){
        rowState[i].cells[j].textContent = field[i][j];
        if (Number(field[i][j]) !== 0) rowState[i].cells[j].classList.add(`field-cell--${field[i][j]}`);
    }
};
game.spawnRandom = function(field) {
    const emptyCells = [];
    for(let i = 0; i < 4; i++){
        for(let j = 0; j < 4; j++)if (Number(field[i][j]) === 0) emptyCells.push([
            i,
            j
        ]);
    }
...
game.checkGameStatus = function() {
    ...
    // Is Win
    for(let i = 0; i < 4; i++){
        for(let j = 0; j < 4; j++)if (Number(field[i][j]) === 2048) {
    ...
    // Is Lose
    for(let i = 0; i < 4; i++){
        for(let j = 0; j < 4; j++)if (Number(field[i][j]) === 0) {
            emptyCells = true;
            break;
        }
    }
    ...
    for(let i = 0; i < 4; i++){
        for(let j = 0; j < 3; j++)if (Number(field[i][j]) === Number(field[i][j + 1])) {
    ...
    for(let i = 0; i < 3; i++){
        for(let j = 0; j < 4; j++)if (Number(field[i][j]) === Number(field[i + 1][j])) {
            isMoveVertical = true;
            break;
        }
    }
...
game.restart = function() {
    ...
    if (isMessageLose) isMessageLose.classList.add("hidden");
    if (isMessageWin) isMessageWin.classList.add("hidden");
    for(let i = 0; i < 4; i++){
        const row = rows[i];
        for(let j = 0; j < 4; j++){
...
document.addEventListener("keydown", (e)=>{
    if (e.key === "ArrowLeft") {
        if (game.getStatus() !== "idle") game.moveLeft();
    }
    if (e.key === "ArrowRight") {
        if (game.getStatus() !== "idle") game.moveRight();
    }
    if (e.key === "ArrowDown") {
        if (game.getStatus() !== "idle") game.moveDown();
    }
    if (e.key === "ArrowUp") {
        if (game.getStatus() !== "idle") game.moveUp();
    }
}); // #endregion
